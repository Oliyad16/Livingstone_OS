...
export async function POST() {
  ...
  // Each upsert is idempotent (keyed on source+ext_id), so a mid-loop failure
  // is safe to retry — report what landed instead of throwing a bare 500.
  try {
    while (hasMore) {
    ...
    return NextResponse.json(
      { error: `Sync stopped after ${synced} charge${synced === 1 ? '' : 's'}: ${message} Run sync again to resume.`, synced },
      { status: 502 }
    )
  }
